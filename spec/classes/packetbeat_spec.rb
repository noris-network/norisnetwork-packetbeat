...
describe 'packetbeat', 'type' => 'class' do
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      it { is_expected.to create_class('packetbeat') }
      it { is_expected.to create_class('packetbeat::install') }
      it { is_expected.to create_class('packetbeat::config') }
      it { is_expected.to create_class('packetbeat::service') }
      ...
      describe 'with disable_configtest false and file permission 0600' do
        ...
        it do
          is_expected.to contain_file('/etc/packetbeat/packetbeat.yml').with(
            'ensure' => 'present',
            'owner' => 'root',
            'group' => 'root',
            'mode' => '0600',
            'validate_cmd' => '/usr/share/packetbeat/bin/packetbeat test config -c %',
          )
        end
      end
